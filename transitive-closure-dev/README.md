...
公开基线是一个多文件 bitset Warshall 工程。可以重写 Makefile、增加或删除源码，也可以采用完全不同的闭包算法。
...
动态库必须导出 `closure_run`。它接收 CPU 输入和输出，一次调用完成显存
...
```bash
nsys profile -o closure_case3 \
  ./build/judge 3 ./build/submission/libclosure_submission.so candidate -
...
建议关注全局矩阵扫描次数、pivot 顺序依赖、L2 命中率、显存吞吐、kernel 启动次数和不同图结构下的表现。
...
`closure_run` 必须使用 `extern "C"`，并与 `interface/closure_api.h`
完全一致。
...
使用输出的 suite seed 复现，重点检查非 64 倍数规模、最后一个 word 的无效位、pivot 顺序和输出完整覆写。
