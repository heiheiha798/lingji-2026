...
void LaunchInitialize(const std::uint64_t *adjacency,
  ...
  const std::size_t bytes =
      static_cast<std::size_t>(vertices) * words * sizeof(std::uint64_t);
  cudaMemcpyAsync(reachability, adjacency, bytes, cudaMemcpyDeviceToDevice,
                  stream);
...
void LaunchPivot(std::uint64_t *reachability, int vertices, int words,
  ...
  WarshallStep<<<vertices, 128, 0, stream>>>(reachability, vertices, words,
                                             pivot);
}
