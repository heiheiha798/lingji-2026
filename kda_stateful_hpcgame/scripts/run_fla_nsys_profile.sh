...
output_dir=$task_dir/nsys_reports/$profile_name
...
"$nsys_bin" stats --quiet --force-export=true \
    --report cuda_gpu_kern_sum --format csv "$report_path" \
    > "$output_dir/kernel-summary.csv"
"$nsys_bin" stats --quiet --report cuda_gpu_trace --format csv "$report_path" \
    > "$output_dir/kernel-trace.csv"
"$nsys_bin" stats --quiet --report cuda_gpu_mem_time_sum --format csv "$report_path" \
    > "$output_dir/memory-summary.csv"
"$nsys_bin" stats --quiet --report nvtx_gpu_proj_sum --format csv "$report_path" \
