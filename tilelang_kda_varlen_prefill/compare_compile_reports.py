...
def main() -> None:
    ...
    try:
        baseline_target, baseline_cases = load_report_group(
            args.baseline, "baseline"
        )
        candidate_target, candidate_cases = load_report_group(
    ...
    if baseline_target != candidate_target:
        parser.error(
            f"target mismatch: {baseline_target!r} != {candidate_target!r}"
        )
        ...
        for stage_name in ("preprocess", "persistent"):
            ...
            for _, metric_name in columns:
                ...
                if not isinstance(baseline_value, int) or not isinstance(
                    ...
                    parser.error(
                        f"{case_name}/{stage_name} has invalid {metric_name}"
                    )
