...
if (( $# < 1 )); then
    echo "usage: $0 EXPERIMENT_NAME [--stage preprocess|diagonal|inter|persistent|both] [K1 K2 K3 K4]" >&2
    exit 2
fi
...
if (( $# >= 1 )) && [[ $1 == --stage ]]; then
    if (( $# < 2 )); then
        echo "--stage requires preprocess, diagonal, inter, persistent, or both" >&2
        exit 2
    fi
    stage=$2
    shift 2
fi
if [[ $stage != preprocess && $stage != diagonal && $stage != inter && $stage != persistent && $stage != both ]]; then
